...
function renderInputs() { const container = $('employee-rows'); container.innerHTML = employees.map((employee) => `<div class="employee-row" data-id="${employee.id}"><input class="employee-name" type="text" placeholder="Employee full name" value="${employee.name.replace(/"/g, '&quot;')}"><div class="row-money"><span>₦</span><input class="employee-salary" type="number" step="any" value="${employee.salary}"></div><button class="remove-employee" type="button" title="Remove employee">×</button></div>`).join(''); $('empty-state').classList.toggle('hidden', employees.length > 0); document.querySelectorAll('.employee-name').forEach((input) => input.addEventListener('input', updateEmployee)); document.querySelectorAll('.employee-salary').forEach((input) => input.addEventListener('input', updateEmployee)); document.querySelectorAll('.remove-employee').forEach((button) => button.addEventListener('click', removeEmployee)); }
